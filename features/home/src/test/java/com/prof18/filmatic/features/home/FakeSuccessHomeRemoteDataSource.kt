...
class FakeSuccessHomeRemoteDataSource : HomeRemoteDataSource {

    val movieModels = listOf(DataFactory.getMovieModel(), DataFactory.getMovieModel())
    val allGenresResponse = DataFactory.getGenreResponse()

    override suspend fun getPopularMovies(): Result<List<MovieModel>> {
        return Result.Success(movieModels)
    }

    override suspend fun getAllGenres(): Result<AllGenresResponse> {
       return Result.Success(allGenresResponse)
    }
}